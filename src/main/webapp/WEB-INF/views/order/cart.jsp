...
<div class="max-w-3xl mx-auto px-6 py-10">
  ...
  <c:choose>
    <c:when test="${empty itemsByRestaurant}">
      <!-- Empty Cart -->
    ...
    <c:otherwise>
      ...
      <c:forEach var="restaurantEntry" items="${itemsByRestaurant}">
        <div class="bg-white rounded-2xl shadow p-6 mb-4">
          ...
          <div class="flex items-center gap-2 mb-4 pb-3 border-b">
            ...
            <span class="ml-auto text-sm text-gray-500">
              🚚 Delivery: Rs.
              ${deliveryFeeByRestaurant[restaurantEntry.key]}
            </span>
          </div>

          <!-- Items for this restaurant -->
          <div class="space-y-4">
            <c:forEach var="cartItem" items="${restaurantEntry.value}">
      ...
      <div class="bg-white rounded-2xl shadow p-6 mb-6">
        <h3 class="font-bold text-gray-800 mb-4">Bill Summary</h3>

        <!-- Food Total -->
        <div class="flex justify-between text-sm text-gray-600 mb-2">
          <span>Food Total</span>
          <span class="font-semibold">Rs. ${foodTotal}</span>
        </div>

        <!-- Delivery fees per restaurant -->
        <c:forEach var="feeEntry" items="${deliveryFeeByRestaurant}">
          <div class="flex justify-between text-sm text-gray-600 mb-2">
            <span>🚚 ${feeEntry.key} Delivery</span>
            <span class="font-semibold">Rs. ${feeEntry.value}</span>
          </div>
        </c:forEach>
        ...
        <!-- Bulk Order Notice -->
        <c:if test="${foodTotal gt 2500}">
          <div class="mt-4 bg-purple-50 border border-purple-200
                      rounded-xl p-3 text-sm text-purple-700">
            🎁 This is a <strong>Bulk Order</strong> —
            10% discount will be applied on food total!
          </div>
        </c:if>
      </div>
